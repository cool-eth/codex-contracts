...
contract ExtraRewardStashTokenRescue {
    ...
    function claimRewardToken(address _token) public returns (bool) {
        require(
            distributor == address(0) || msg.sender == distributor,
            "!distributor"
        );
        ...
        if (amount > 0) {
            if (
                rewardDeposit != address(0) &&
                CheckOption(
                    activeTokens[_token],
                    uint256(Options.SendToRewards)
                )
            ) {
    ...
    function setExtraReward(address _token, uint256 _option) external {
        ...
        if (
            CheckOption(_option, uint256(Options.SendToRewards)) &&
            rewardDeposit != address(0)
        ) {
            IERC20(_token).safeApprove(rewardDeposit, 0);
            IERC20(_token).safeApprove(rewardDeposit, type(uint256).max);
        }
