...
//Stash v3: support for bunni gauge reward redirect
...
contract ExtraRewardStashV3 {
    ...
    function claimRewards() external returns (bool) {
        ...
        uint256 boosterOLIT = IERC20(oLIT).balanceOf(operator);
        ...
        if (!hasRedirected) {
            ...
            //always make sure cdx is added as well
            setToken(cdx);
        }
        ...
        require(boosterOLIT == IERC20(oLIT).balanceOf(operator), "oLITChange");
        return true;
    }
    ...
    // (any new incentive that is not directly on bunni gauges)
    ...
    function setToken(address _token) internal {
        ...
        if (t.token == address(0)) {
            ...
            if (_token != oLIT) {
                //create new wrapper
                address tokenwrapper = IProxyFactory(proxyFactory).clone(
                    tokenWrapperImplementation
                );

                //create new reward contract (for NON-oLIT tokens only)
    ...
    function processStash() external returns (bool) {
        ...
        for (uint i = 0; i < tCount; i++) {
            ...
            if (token == oLIT) {
                //if oLIT, send back to booster to distribute
