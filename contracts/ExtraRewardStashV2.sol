        ...
        if (length > 0) {
            ...
            for (uint256 i = 0; i < length; i++) {
                ...
                if (newbalance > balances[i]) {
                    ...
                    uint256 activeCount = IRewardFactory(rewardFactory)
                        .activeRewardCount(token);
                    if (activeCount > 1) {
                        //send to arbitrator
                    ...
                    if (block.timestamp > tokenInfo[i].lastActiveTime + WEEK) {
                        //set as inactive
                        IRewardFactory(rewardFactory).removeActiveReward(
                            token,
                            pid
                        );
                    } else {
    ...
    function setToken(uint256 _tid, address _token) internal {
        ...
        if (currentToken != _token) {
            ...
            address rewardContract = IRewardFactory(rewardFactory)
                .CreateTokenRewards(_token, mainRewardContract, address(this));
